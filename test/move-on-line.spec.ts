...
import { parseLocation, lerp, move } from '../src/move-on-line'
...
test('指定距離まで移動したときの座標', t => {
  t.deepEqual(move([3, 4], [0, 0], 1), {
    diff: -4,
    coords: [2.4, 3.2]
  })

  t.deepEqual(move([0, 0], [3, 4], 1), {
    diff: -4,
    coords: [0.6, 0.8]
  })
})
...
test('指定距離まで移動して終点を超えた場合', t => {
  t.deepEqual(move([3, 4], [0, 0], 6), {
    diff: 1,
    coords: [0, 0]
  })

  t.deepEqual(move([0, 0], [3, 4], 7), {
    diff: 2,
    coords: [3, 4]
  })
})
